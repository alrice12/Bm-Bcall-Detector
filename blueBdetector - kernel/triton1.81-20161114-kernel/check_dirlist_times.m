...
elseif fflag == 0   % all *.head.hrp in directory
    PARAMS.headall = [];
    PARAMS.inpath = uigetdir(PARAMS.inpath,'Select Directory with *head.hrp files');
    ...
    nfiles = fnsz(1);
%     disp_msg(' ')
%     disp_msg([num2str(nfiles),'  *head.hrp files'])
    if nfiles < 1
        disp_msg(['No data files in this directory: ',indir])
