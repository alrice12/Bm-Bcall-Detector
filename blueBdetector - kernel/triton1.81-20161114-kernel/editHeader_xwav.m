...
dlgTitle=['Modify XWAV Header ',PARAMS.inpath,PARAMS.infile];
...
% make sure want to change xwav header timing values
button = questdlg('Are you sure you want to change the XWAV header values?',...
    'Change XWAV header???','Yes','No','No');

if strcmp(button,'No')
    disp_msg('Cancel XWAV header change')
    return
end
...
fod = fopen([PARAMS.inpath,PARAMS.infile],'r+');
...
disp_msg(['Finished modifying XWAV File: ',PARAMS.inpath,PARAMS.infile])
