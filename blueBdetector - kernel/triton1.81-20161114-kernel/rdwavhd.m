...
%     PARAMS.xhd.dSubchunkSize = fread(fid,1,'uint32');        % (Size of Subchunk - 8) includes write subchunk
%     
%     if ~strcmp(PARAMS.xhd.dSubchunkID,'data') || PARAMS.xhd.dSubchunkSize <= 0
%         disp_msg('hummm, should be "data" here?')
%         disp_msg(['SubchunkID = ',PARAMS.xhd.dSubchunkID])
%     end
%     %return
...
% function specialwavread
% 
% global PARAMS
% 
% [y, Fs, nBits, OPTS] = wavread( [PARAMS.inpath PARAMS.infile],'size');
% PARAMS.xhd.NumChannels = y(2);
% PARAMS.xhd.SampleRate = Fs;
% PARAMS.xhd.ByteRate = Fs * PARAMS.xhd.NumChannels * floor(nBits/8);       % Byte Rate = SampleRate * NumChannels * BitsPerSample / 8
% PARAMS.xhd.BlockAlign = PARAMS.xhd.NumChannels * floor(nBits/8);          % # of Bytes per Sample Slice = NumChannels * BitsPerSample / 8
% PARAMS.xhd.BitsPerSample = nBits;       % # of Bits per Sample : 8bit = 8, 16bit = 16, etc
% PARAMS.xhd.dSubchunkID = 'data';
% PARAMS.xhd.dSubchunkSize = y(1) * PARAMS.xhd.NumChannels * floor(nBits/8);

% PARAMS.xhd.dSubchunkSize =        PARAMS.xhd.BitsPerSample/8 % NumSamples * NumChannels * BitsPerSample/8