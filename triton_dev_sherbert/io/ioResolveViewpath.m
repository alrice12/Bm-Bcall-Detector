...
% 
% Optargs are file opening arguments: permission, machineformat, and
% encoding, see fopen for details.  If not specified, the file is opened
...
if nargin < 3 
    ReadOnly = true;
elseif sum(strcmp(varargin{1}, {'r', 'rt'}))
    ReadOnly = true;
else
    ReadOnly = false;
end
...
while ~ found && vdir > 0
    found = exist(fullfile(viewpath{vdir}, fname), 'file');
    if ~ found
        vdir = vdir - 1;
    end
end
