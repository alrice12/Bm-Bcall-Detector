function [Starts, Stops, Labels] = ioReadLabelFile(Filename, varargin)
...
%       format.
%  'Viewpath', {dir1, dir2, ..., dirN} - Given a cell array of directories,
%       search the directory list for Filename relative to each of the
%       directories.
%
% Do not modify the following line, maintained by CVS
% $Id: ioReadLabelFile.m,v 1.7 2014/03/03 17:01:50 mroch Exp $
...
while vidx <= length(varargin)
  switch varargin{vidx}
   ...
   case 'Viewpath'
    ...
    if ~ iscell(Viewpath)
        error('Viewpath must be a cell array of directory names');
    end
    vidx = vidx + 2;
   otherwise
...
fileh = ioResolveViewpath(Filename, Viewpath);
if fileh < 1
    dirs = sprintf('"%s" ', Viewpath{:});
    error('io:open', 'Unable to find %s on viewpath %s', Filename, dirs);
end
...
while moretoread
  ...
  if ~ isempty(Start) && ~ isempty(Stop)
    ...
    while ~ isempty(nextchar) && nextchar ~= linefeed
