...
from http://www.eclipse.org/tptp/home/downloads/#agentController
...
Heap dump and profiling tool VisualVM -----------------------------------

First, you need to create or edit the java.opts file.
...
Regardless of where you choose to have the java.opts file, add the following line:
-XX:HeapDumpPath=PATH
-XX:+HeapDumpOnOutOfMemoryError

where, PATH is location where you want to save .hprof file.
For example, -XX:HeapDumpPath=./heapdump.hprof

The -XX:+HeapDumpOnOutOfMemoryError option generates a heap dump 
when an allocation from the Java heap or the permanent generation cannot be 
satisfied. There is no overhead in running with this option.The heap dump is 
in HPROF binary format, and so it can be analyzed using any tools that can 
import this format.

VisualVM: The VisualVM is very good tool for analyzing the heap dump and for  
profiling. VisualVM is integrated with JDK 6 update 7 release. A binary  
launcher 'jvisualvm' is put in the ${JDK}/bin directory along with the rest of 
the tools. In our case, it is put in /usr/bin/.
